...
const requireAuth = async (req, res, next) => {
    ...
    let decodedToken;
    if (jwtToken) decodedToken = await verify(jwtToken, process.env.JWT_SECRET);
    
    let institutionCode;
    institutionCode = jwtToken
        ? decodedToken["institution_code"]
        : req.query["institution_code"];
    // rows 是一個包含查詢結果的陣列
...
const checkUser = async (req, res, next) => {
    ...
    if (jwtToken) {
        try {
            ...
            const userInfo = await knex(rows[0].table).select("account", "name", "balance").where("account", decodedToken.account).first();
